...
class NodeSimpleGimbalSolarPanelInputs(NamedTuple):
    in_solar_constant: InputPort
    in_sun_unit_sc: InputPort
    in_sp_gimbal_angle: InputPort
    in_is_occluded: InputPort
    input_q_sc2eci: InputPort

class NodeSimpleGimbalSolarPanelOutputs(NamedTuple):
    out_power: OutputPort
    out_gimbal: OutputPort
    out_gimbal_measure: OutputPort

class NodeSimpleGimbalSolarPanel(NodeDifferential):
    def __init__(self, **kwargs):
        self._i = NodeSimpleGimbalSolarPanelInputs(
            InputPort("in_solar_constant", self),
            InputPort("in_sun_unit_sc", self),
            InputPort("in_sp_gimbal_angle", self),
            InputPort("in_is_occluded", self),
            InputPort("input_q_sc2eci", self)
        )
        self._o = NodeSimpleGimbalSolarPanelOutputs(
            OutputPort("out_power", self),
            OutputPort("out_gimbal", self),
            OutputPort("out_gimbal_measure", self)
        )

        super().__init__(np.array([0.0]), self._i, self._o, **kwargs)
    ...
    def update(self, sim_time: float):
        in_sun_unit_sc = self._i.in_sun_unit_sc.read()
        ...
        in_sp_gimbal_angle = np.clip(
            self._i.in_sp_gimbal_angle.read()
            if self._i.in_sp_gimbal_angle.read() != None
            else 0,
            -self._gimbal_max_angle,
            self._gimbal_max_angle,
        )
        solar_constant = self._i.in_solar_constant.read()
        is_occluded = self._i.in_is_occluded.read()
        ...
        if is_occluded == False:
            ...
            self._o.out_power.shift_out(
                proj_area * solar_constant * self._efficiency / self._panel_voltage
            )
        else:
            self._o.out_power.shift_out(0)

        self._o.out_gimbal.shift_out(self._x)
        self._o.out_gimbal_measure.shift_out(
