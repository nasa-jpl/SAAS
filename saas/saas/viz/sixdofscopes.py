...
class NodeSCTrajScopeInputs(NamedTuple):
    input_r_eci: InputPort
    input_v_eci: InputPort
    input_sc_sun_icrs: InputPort
    input_sc_earth_icrs: InputPort
    input_earth_occ: InputPort
    input_sun_occ: InputPort
    in_start_datetime: InputPort

class NodeSCTrajScope(Node):
    def __init__(self, **kwargs):
        self._i = NodeSCTrajScopeInputs(
            InputPort("input_r_eci", self),
            InputPort("input_v_eci", self),
            InputPort("input_sc_sun_icrs", self),
            InputPort("input_sc_earth_icrs", self),
            InputPort("input_earth_occ", self),
            InputPort("input_sun_occ", self),
    ...
    def update(self, sim_time: float):
        r = self._i.input_r_eci.read()
        v = self._i.input_v_eci.read()

        self._t.append(sim_time)
        self._r.append(r)
        self._v.append(v)

        e = self._i.input_sc_earth_icrs.read()
        if np.any(e) == None:
            self._e.append(np.zeros((3,)))
        else:
            self._e.append(e)

        s = self._i.input_sc_sun_icrs.read()
        if np.any(s) == None:
        ...
        occ = (
            self._i.input_sun_occ.read(),
            self._i.input_earth_occ.read(),
        )
...
class NodeSCAttitudeScopeInputs(NamedTuple):
    input_q_sc_to_eci: InputPort
    input_w_sc: InputPort
    input_nadir: InputPort
    input_gimbal: InputPort
    input_sun_unit: InputPort
    input_earth_unit: InputPort
    input_fs_state: InputPort
    input_earth_occ: InputPort
    input_sun_occ: InputPort
    in_start_datetime: InputPort
...
class NodeSCAttitudeScope(Node):
    def __init__(self, config: str = None):
        self._i = NodeSCAttitudeScopeInputs(
            InputPort("input_q_sc_to_eci", self),
            InputPort("input_w_sc", self),
            InputPort("input_nadir", self),
            InputPort("input_gimbal", self),
            InputPort("input_sun_unit", self),
            InputPort("input_earth_unit", self),
            InputPort("input_fs_state", self),
            InputPort("input_earth_occ", self),
            InputPort("input_sun_occ", self),
            InputPort("in_start_datetime", self)
        )

        super().__init__(self._i, (), config)
    ...
    def update(self, sim_time: float):
        q = self._i.input_q_sc_to_eci.read()
        if np.any(q) == None:
            self._q.append(Rotation.identity().as_quat(canonical=True))
        else:
            self._q.append(q)

        w = self._i.input_w_sc.read()
        if np.any(w) == None:
            self._w.append(np.zeros((3,)))
        else:
            self._w.append(w)

        n = self._i.input_nadir.read()
        if np.any(n) == None:
            self._n.append(np.zeros((3,)))
        else:
            self._n.append(n)

        g = np.copy(self._i.input_gimbal.read())
        if g == None:
            self._g.append(np.array([0]))
        else:
            self._g.append(g)

        e = self._i.input_earth_unit.read()
        if np.any(e) == None:
            self._e.append(np.zeros((3,)))
        else:
            self._e.append(e)

        s = self._i.input_sun_unit.read()
        if np.any(s) == None:
            self._s.append(np.zeros((3,)))
        else:
            self._s.append(s)

        self._fs_state.append(self._i.input_fs_state.read())
